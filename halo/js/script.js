...
function searchBarToggler() {
  if (searchBarStatus == 0) {
    document.getElementsByClassName("search-bar")[0].style.visibility =
      "visible";
...
if (typeof Storage !== "undefined") {
  ...
  */
  ...
  regBtn.click(() => {
    ...
    let accountsArray = JSON.parse(
      localStorage.getItem("HALO accounts storage")
    );
  ...
  logBtn.click(() => {
    ...
    for (let i = 0; i < accountsArray.length; i++) {
      ...
      if (
        accountsArray[i].userMail == emailLoginInput &&
        ...
        logregTogglers.html(JSON.parse(localStorage.currentUser).userName);
...
if (mainInfoToggler.length > 0) {
  function mainInfoIndicatorToggler1() {
    if (
      this.childNodes[1].childNodes[0].className.match(
        /(?:^|\s)fa-chevron-up(?!\S)/
      )
    ) {
  ...
  function mainInfoIndicatorToggler2() {
    if (
      this.childNodes[1].childNodes[1].className.match(
        /(?:^|\s)fa-chevron-up(?!\S)/
      )
    ) {
...
function checkEmptyCart() {
  let checkEmptyGrandTotal =
    document.getElementsByClassName("cart__grand-total")[0].innerHTML;
...
for (let i = 0; i < removeItemBtn.length; i++) {
  removeItemBtn[i].addEventListener("click", function () {
    ...
    if (
      confirm(
        `Quý khách muốn xóa sản phẩm ${this.parentElement
...
if (inputCVV != null) {
  ...
  inputCVV.addEventListener("focus", function () {
    //quay thẻ ra sau
    document.querySelectorAll(".payment-card__content")[1].style.transform =
      "rotateY(180deg)";
  });
}
...
//Xác định ô input
let inputValue = document.querySelectorAll(
  ".payment-card__input .form-control"
);
...
if (inputValue.length != 0) {
  ...
  for (let i = 0; i < notNumOrBank.length; i++) {
    ...
    inputValue[notNumOrBank[i]].addEventListener("input", function () {
      ...
      displayValue[notNumOrBank[i] - 1].innerHTML =
        inputValue[notNumOrBank[i]].value.toUpperCase();
    });
  }
}
...
$(window).on("scroll", function () {
  if (!isFacebookLoaded) {
    let hT = $(".intro--lens").offset().top,
      hH = $(".intro--lens").outerHeight(),
      wH = $(window).height(),
      wS = $(this).scrollTop();
    if (wS > hT + hH - wH) {
      $("body").append(`
...
// .fb-customerchat(attribution='setup_tool' page_id='103550388473714' theme_color='#6699cc' logged_in_greeting='Xin chào, HALO có thể giúp gì cho bạn?' logged_out_greeting='Xin chào, HALO có thể giúp gì cho bạn?')
